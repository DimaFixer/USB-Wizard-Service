                      ...
                      + "'} WHERE AssocClass = Win32_DiskDriveToDiskPartition").Get())
                {
                 ...
                 new System.Management.ManagementObjectSearcher(
                          ...
                          + "'} WHERE AssocClass = Win32_LogicalDiskToPartition").Get())
                    {
                        try
                        {
                            USBInfo MyUSB = new USBInfo(parseSerialFromDeviceID(drive["PNPDeviceID"].ToString().Trim()));
                            if (File.Exists(disk["Name"].ToString().Trim() + "\\Licence.key"))
                            {
                                ...
                                try
                                {
                                    //Переделать путь после завершения работы над программой???
                                    //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                                    RegistryKey myKey = Registry.LocalMachine.OpenSubKey(@"", false);
                                    ...
                                    Task.Factory.StartNew(() =>
                                    {
                                        ...
                                        {
                                            using (NamedPipeServerStream pipe = new NamedPipeServerStream("KeyKeeper", PipeDirection.InOut, 1, PipeTransmissionMode.Message, PipeOptions.None, 2048, 2048, ps))
                                            {
